    ...
    public void verifyShippersMenuItemsAndTheirContent(DataTable dataTable) {
        ...
        for (Map<String, String> row : data) {
            ...
            try {
                ...
                // Wait for the document to be fully loaded using the global WebDriverWait
                WebDriverWait wait = BrowserUtils.getWait();
                ...
                // Verify heading
                WebElement heading;
                if (menuItem.equals("i-Tools Login")) {
                    // Special handling for i-Tools Login page
                    heading = driver().findElement(By.xpath("//h3[contains(text(), 'i-Tools')]"));
                } else {
                    // Default handling for other pages
                    heading = driver().findElement(By.cssSelector("h1.x-text-content-text-primary"));
                }
